...
class t_csharp5_generator: public t_oop_generator {
public:
	t_csharp5_generator(t_program* program,
	...
	void generate_property(ofstream& out, t_field* tfield, bool isPublic,
			bool generateIsset);
	void generate_csharp_property(ofstream& out, t_field* tfield, bool isPublic,
			bool includeIsset = true, std::string fieldPrefix = "");
	bool print_const_value(std::ofstream& out, std::string name, t_type* type,
			t_const_value* value, bool in_static, bool defval = false,
			bool needtype = false);
	std::string render_const_value(std::ofstream& out, std::string name,
			t_type* type, t_const_value* value);
	void print_const_constructor(std::ofstream& out,
			std::vector<t_const*> consts);
	void print_const_def_value(std::ofstream& out, std::string name,
			t_type* type, t_const_value* value);
	...
	void generate_csharp_struct_definition(std::ofstream& out,
			t_struct* tstruct, bool is_xception = false, bool in_class = false,
	...
	void generate_csharp_struct_result_writer(std::ofstream& out,
			t_struct* tstruct);
	...
	void generate_deserialize_field(std::ofstream& out, t_field* tfield,
			std::string prefix = "", bool is_propertyless = false);
	...
	void generate_deserialize_map_element(std::ofstream& out, t_map* tmap,
			std::string prefix = "");
	void generate_deserialize_list_element(std::ofstream& out, t_list* list,
			std::string prefix = "");
	void generate_serialize_field(std::ofstream& out, t_field* tfield,
			std::string prefix = "", bool is_element = false,
			bool is_propertyless = false);
	void generate_serialize_struct(std::ofstream& out, t_struct* tstruct,
			std::string prefix = "");
	void generate_serialize_container(std::ofstream& out, t_type* ttype,
			std::string prefix = "");
	void generate_serialize_map_element(std::ofstream& out, t_map* tmap,
			std::string iter, std::string map);
	void generate_serialize_set_element(std::ofstream& out, t_set* tmap,
			std::string iter);
	void generate_serialize_list_element(std::ofstream& out, t_list* tlist,
			std::string iter);
	...
	std::string type_name(t_type* ttype, bool in_countainer = false,
			bool in_init = false, bool in_param = false, bool is_required =
					false);
	std::string base_type_name(t_base_type* tbase, bool in_container = false,
			bool in_param = false, bool is_required = false);
	std::string declare_field(t_field* tfield, bool init = false,
			std::string prefix = "");
	std::string function_signature_async_begin(t_function* tfunction,
			std::string prefix = "");
	std::string function_signature_async_end(t_function* tfunction,
			std::string prefix = "");
...
void t_csharp5_generator::init_generator() {
...
void t_csharp5_generator::start_csharp_namespace(ofstream& out) {
...
void t_csharp5_generator::end_csharp_namespace(ofstream& out) {
	if (!namespace_name_.empty()) {
		scope_down(out);
	}
}

string t_csharp5_generator::csharp_type_usings() {
...
string t_csharp5_generator::csharp_thrift_usings() {
	return string() + "using Thrift.Protocol;\n" + "using Thrift.Transport;\n";
}

void t_csharp5_generator::close_generator() {
}
void t_csharp5_generator::generate_typedef(t_typedef* ttypedef) {
	(void) ttypedef;
}

void t_csharp5_generator::generate_enum(t_enum* tenum) {
...
void t_csharp5_generator::generate_consts(std::vector<t_const*> consts) {
	if (consts.empty()) {
		return;
	}
	...
	for (c_iter = consts.begin(); c_iter != consts.end(); ++c_iter) {
		...
		if (print_const_value(f_consts, (*c_iter)->get_name(),
				(*c_iter)->get_type(), (*c_iter)->get_value(), false)) {
...
void t_csharp5_generator::print_const_def_value(std::ofstream& out, string name,
		t_type* type, t_const_value* value) {
	if (type->is_struct() || type->is_xception()) {
		...
		for (v_iter = val.begin(); v_iter != val.end(); ++v_iter) {
			...
			string val = render_const_value(out, name, field_type,
					v_iter->second);
			indent(out) << name << "." << prop_name(field) << " = " << val
					<< ";" << endl;
		}
	} else if (type->is_map()) {
...
void t_csharp5_generator::print_const_constructor(std::ofstream& out,
		std::vector<t_const*> consts) {
...
bool t_csharp5_generator::print_const_value(std::ofstream& out, string name,
...
std::string t_csharp5_generator::render_const_value(ofstream& out, string name,
		t_type* type, t_const_value* value) {
	(void) name;
...
void t_csharp5_generator::generate_struct(t_struct* tstruct) {
	if (tstruct->is_union()) {
...
void t_csharp5_generator::generate_xception(t_struct* txception) {
	generate_csharp_struct(txception, true);
}

void t_csharp5_generator::generate_csharp_struct(t_struct* tstruct,
		bool is_exception) {
...
void t_csharp5_generator::generate_csharp_struct_definition(ofstream &out,
	...
	if (wcf_ && !is_exception) {
		indent(out) << "[DataContract(Namespace=\"" << wcf_namespace_ << "\")]"
				<< endl; // do not make exception classes directly WCF serializable, we provide a seperate "fault" for that
	}
	...
	for (m_iter = members.begin(); m_iter != members.end(); ++m_iter) {
		...
		if (!field_is_required((*m_iter)) && field_has_default((*m_iter))) {
			indent(out) << "private " << declare_field(*m_iter, false, "_")
					<< endl;
		}
	}
	out << endl;
	...
	bool generate_isset = has_non_required_default_value_fields;
	if (generate_isset) {
		out << endl << indent() << "public Isset __isset;" << endl << indent()
				<< "#if !SILVERLIGHT" << endl << indent() << "[Serializable]"
				<< endl << indent() << "#endif" << endl;
		if (wcf_) {
		...
		for (m_iter = members.begin(); m_iter != members.end(); ++m_iter) {
			...
			if (!is_required && has_default) {
				indent(out) << "public bool " << (*m_iter)->get_name() << ";"
						<< endl;
			}
		}
	...
	for (m_iter = members.begin(); m_iter != members.end(); ++m_iter) {
		...
		if ((*m_iter)->get_value() != NULL) {
			if (field_is_required((*m_iter))) {
				...
				print_const_value(out, "this._" + (*m_iter)->get_name(), t,
						(*m_iter)->get_value(), true, true);
...
void t_csharp5_generator::generate_csharp_wcffault(ofstream& out,
		t_struct* tstruct) {
	out << endl;
	indent(out) << "#if !SILVERLIGHT" << endl;
	indent(out) << "[Serializable]" << endl;
...
void t_csharp5_generator::generate_csharp_struct_reader(ofstream& out,
		t_struct* tstruct) {
	...
	indent(out) << "TField field;" << endl << indent()
	...
	for (f_iter = fields.begin(); f_iter != fields.end(); ++f_iter) {
		if (field_is_required((*f_iter))) {
			indent(out) << "if (!isset_" << (*f_iter)->get_name() << ")"
					<< endl;
			indent_up();
			indent(out)
...
void t_csharp5_generator::generate_csharp_struct_writer(ofstream& out,
		t_struct* tstruct) {
	...
	if (fields.size() > 0) {
		...
		for (f_iter = fields.begin(); f_iter != fields.end(); ++f_iter) {
			...
			if (!has_default && !is_required) {
				indent(out) << "if (" << prop_name((*f_iter)) << " != null) {"
						<< endl;
				indent_up();
			} else if (!is_required) {
				bool null_allowed = type_can_be_null((*f_iter)->get_type());
				if (null_allowed) {
					indent(out) << "if (" << prop_name((*f_iter))
							<< " != null && __isset." << (*f_iter)->get_name()
							<< ") {" << endl;
					indent_up();
				} else {
					indent(out) << "if (__isset." << (*f_iter)->get_name()
							<< ") {" << endl;
					indent_up();
				}
			}
			indent(out) << "field.Name = \"" << (*f_iter)->get_name() << "\";"
					<< endl;
			indent(out) << "field.Type = "
					<< type_to_enum((*f_iter)->get_type()) << ";" << endl;
...
void t_csharp5_generator::generate_csharp_struct_result_writer(ofstream& out,
		t_struct* tstruct) {
	...
	if (fields.size() > 0) {
		...
		for (f_iter = fields.begin(); f_iter != fields.end(); ++f_iter) {
			...
			indent(out) << "field.Name = \"" << prop_name(*f_iter) << "\";"
					<< endl;
			indent(out) << "field.Type = "
					<< type_to_enum((*f_iter)->get_type()) << ";" << endl;
			indent(out) << "field.ID = " << (*f_iter)->get_key() << ";" << endl;
...
void t_csharp5_generator::generate_csharp_struct_tostring(ofstream& out,
		t_struct* tstruct) {
	...
	for (f_iter = fields.begin(); f_iter != fields.end(); ++f_iter) {
		if (first) {
			first = false;
			indent(out) << "sb.Append(\"" << prop_name((*f_iter)) << ": \");"
					<< endl;
		} else {
			indent(out) << "sb.Append(\"," << prop_name((*f_iter)) << ": \");"
					<< endl;
		...
		if (ttype->is_xception() || ttype->is_struct()) {
			...
			indent(out) << "sb.Append(" << prop_name((*f_iter)) << ");" << endl;
		}
	}
...
void t_csharp5_generator::generate_csharp_union(t_struct* tunion) {
	string f_union_name = namespace_dir_ + "/" + (tunion->get_name()) + ".cs";
	ofstream f_union;

	f_union.open(f_union_name.c_str());

	f_union << autogen_comment() << csharp_type_usings()
			<< csharp_thrift_usings() << endl;
...
void t_csharp5_generator::generate_csharp_union_definition(std::ofstream& out,
		t_struct* tunion) {
	...
	indent(out) << "public class ___undefined : " << tunion->get_name() << " {"
			<< endl;
	indent_up();
	...
	indent_up();
	indent(out)
			<< "throw new Exception(\"Cannot persist an union type which is not set.\");"
			<< endl;
	indent_down();
	indent(out) << "}" << endl << endl;
...
void t_csharp5_generator::generate_csharp_union_class(std::ofstream& out,
		t_struct* tunion, t_field* tfield) {
	...
	indent_up();
	indent(out) << "TStruct struc = new TStruct(\"" << tunion->get_name()
			<< "\");" << endl;
	indent(out) << "oprot.WriteStructBegin(struc);" << endl;

	indent(out) << "TField field = new TField();" << endl;
	indent(out) << "field.Name = \"" << tfield->get_name() << "\";" << endl;
	indent(out) << "field.Type = " << type_to_enum(tfield->get_type()) << ";"
			<< endl;
...
void t_csharp5_generator::generate_csharp_struct_equals(ofstream& out,
		t_struct* tstruct) {
	...
	for (f_iter = fields.begin(); f_iter != fields.end(); ++f_iter) {
		...
		if (!field_is_required((*f_iter)) && field_has_default((*f_iter))) {
			out << "((__isset." << (*f_iter)->get_name() << " == other.__isset."
					<< (*f_iter)->get_name() << ") && ((!__isset."
					<< (*f_iter)->get_name() << ") || (";
		}
		t_type* ttype = (*f_iter)->get_type();
		if (ttype->is_container()) {
			out << "TCollections.Equals(" << prop_name((*f_iter)) << ", other."
					<< prop_name((*f_iter)) << ")";
		} else {
			out << prop_name((*f_iter)) << ".Equals(other."
					<< prop_name((*f_iter)) << ")";
		}
		if (!field_is_required((*f_iter)) && field_has_default((*f_iter))) {
			out << ")))";
...
void t_csharp5_generator::generate_csharp_struct_hashcode(ofstream& out,
		t_struct* tstruct) {
...
void t_csharp5_generator::generate_service(t_service* tservice) {
	...
	indent(f_service_) << "public partial class " << service_name_ << " {"
			<< endl;
	indent_up();
...
void t_csharp5_generator::generate_service_interface(t_service* tservice) {
	string extends = "";
	...
	for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
		...
		if (wcf_) {
			...
			for (x_iter = xceptions.begin(); x_iter != xceptions.end();
					++x_iter) {
				indent(f_service_)
						<< "[FaultContract(typeof("
								+ type_name((*x_iter)->get_type(), false, false)
								+ "Fault))]" << endl;
			}
		}

		indent(f_service_) << function_signature(*f_iter) << ";" << endl;
		indent(f_service_) << function_signature_async_begin(*f_iter, "Begin_")
				<< ";" << endl;
		indent(f_service_) << function_signature_async_end(*f_iter, "End_")
				<< ";" << endl;
...
void t_csharp5_generator::generate_service_helpers(t_service* tservice) {
	vector<t_function*> functions = tservice->get_functions();
	vector<t_function*>::iterator f_iter;
...
void t_csharp5_generator::generate_service_client(t_service* tservice) {
	string extends = "";
	...
	indent(f_service_) << "public class Client : " << extends_client
			<< "Iface {" << endl;
	indent_up();
	...
	scope_up(f_service_);
	if (extends.empty()) {
		f_service_ << indent() << "iprot_ = iprot;" << endl << indent()
				<< "oprot_ = oprot;" << endl;
	}
	scope_down(f_service_);
	...
	if (extends.empty()) {
		f_service_ << indent() << "protected TProtocol iprot_;" << endl
				<< indent() << "protected TProtocol oprot_;" << endl << indent()
	...
	for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
		...
		scope_up(f_service_);
		indent(f_service_) << "return " << "send_" << funname
				<< "(callback, state";
		...
		// async
		bool first;
		indent(f_service_) << "public async "
				<< function_signature_async(*f_iter, "") << endl;
		scope_up(f_service_);

		if (!(*f_iter)->get_returntype()->is_void()) {
			indent(f_service_) << type_name((*f_iter)->get_returntype())
					<< " retval;" << endl;
	...
	indent(f_service_) << "var asyncResult = Begin_" << funname
			<< "(null, null";
	for (fld_iter = fields.begin(); fld_iter != fields.end(); ++fld_iter) {
		f_service_ << ", " << (*fld_iter)->get_name();
	}
	...
	t_function send_function(g_type_void,
			string("send_") + (*f_iter)->get_name(), (*f_iter)->get_arglist());
	...
	f_service_ << indent() << "oprot_.WriteMessageBegin(new TMessage(\""
			<< funname << "\", TMessageType.Call, seqid_));" << endl << indent()
			<< argsname << " args = new " << argsname << "();" << endl;
	...
	indent(f_service_) << "return oprot_.Transport.BeginFlush(callback, state);"
			<< endl;
	...
	if (!(*f_iter)->is_oneway()) {
		...
		f_service_ << indent()
				...
				<< endl << indent() << "iprot_.ReadMessageEnd();" << endl
				<< indent() << "throw x;" << endl;
		indent_down();
		f_service_ << indent() << "}" << endl << indent() << resultname
				<< " result = new " << resultname << "();" << endl << indent()
				<< "result.Read(iprot_);" << endl << indent()
				<< "iprot_.ReadMessageEnd();" << endl;

		if (!(*f_iter)->get_returntype()->is_void()) {
			if (type_can_be_null((*f_iter)->get_returntype())) {
				f_service_ << indent() << "if (result.Success != null) {"
						<< endl << indent() << "  return result.Success;"
						<< endl << indent() << "}" << endl;
			} else {
				f_service_ << indent() << "if (result.Success.HasValue) {"
						<< endl << indent() << "  return result.Success.Value;"
						<< endl << indent() << "}" << endl;
			}
		}
		...
		for (x_iter = xceptions.begin(); x_iter != xceptions.end(); ++x_iter) {
			f_service_ << indent() << "if (result." << prop_name(*x_iter)
					<< " != null) {" << endl << indent() << "  throw result."
					<< prop_name(*x_iter) << ";" << endl << indent() << "}"
					<< endl;
		}
...
void t_csharp5_generator::generate_service_server(t_service* tservice) {
vector<t_function*> functions = tservice->get_functions();
vector<t_function*>::iterator f_iter;
...
f_service_ << indent() << "ProcessFunction fn;" << endl << indent()
		<< "processMap_.TryGetValue(msg.Name, out fn);" << endl << indent()
		...
		<< "  oprot.Transport.Flush();" << endl << indent() << "  return true;"
...
void t_csharp5_generator::generate_function_helpers(t_function* tfunction) {
...
void t_csharp5_generator::generate_process_function(t_service* tservice,
...
if (!tfunction->is_oneway() && xceptions.size() > 0) {
	...
	for (x_iter = xceptions.begin(); x_iter != xceptions.end(); ++x_iter) {
		...
		if (!tfunction->is_oneway()) {
			indent_up();
			f_service_ << indent() << "result." << prop_name(*x_iter) << " = "
					<< (*x_iter)->get_name() << ";" << endl;
			indent_down();
			f_service_ << indent() << "}";
		} else {
...
f_service_ << indent() << "oprot.WriteMessageBegin(new TMessage(\""
		...
		<< indent() << "result.Write(oprot);" << endl << indent()
		<< "oprot.WriteMessageEnd();" << endl << indent()
...
void t_csharp5_generator::generate_csharp_union_reader(std::ofstream& out,
	t_struct* tunion) {
// Thanks to THRIFT-1768, we don't need to check for required fields in the union
const vector<t_field*>& fields = tunion->get_members();
vector<t_field*>::const_iterator f_iter;

indent(out) << "public static " << tunion->get_name()
		<< " Read(TProtocol iprot)" << endl;
scope_up(out);
...
for (f_iter = fields.begin(); f_iter != fields.end(); ++f_iter) {
	...
	indent(out) << "retval = new " << (*f_iter)->get_name() << "(temp);"
			<< endl;

	indent_down();
	out << indent() << "} else { " << endl << indent()
			<< "  TProtocolUtil.Skip(iprot, field.Type);" << endl << indent()
			<< "  retval = new ___undefined();" << endl << indent() << "}"
...
scope_up(out);
indent(out) << "throw new TProtocolException(TProtocolException.INVALID_DATA);"
		<< endl;
scope_down(out);
...
void t_csharp5_generator::generate_deserialize_field(ofstream& out,
	t_field* tfield, string prefix, bool is_propertyless) {
...
void t_csharp5_generator::generate_deserialize_struct(ofstream& out,
	t_struct* tstruct, string prefix) {
if (tstruct->is_union()) {
...
void t_csharp5_generator::generate_deserialize_container(ofstream& out,
	t_type* ttype, string prefix) {
scope_up(out);
...
void t_csharp5_generator::generate_deserialize_map_element(ofstream& out,
	t_map* tmap, string prefix) {
string key = tmp("_key");
string val = tmp("_val");
...
void t_csharp5_generator::generate_deserialize_set_element(ofstream& out,
	t_set* tset, string prefix) {
...
void t_csharp5_generator::generate_deserialize_list_element(ofstream& out,
	t_list* tlist, string prefix) {
string elem = tmp("_elem");
...
void t_csharp5_generator::generate_serialize_field(ofstream& out,
	t_field* tfield, string prefix, bool is_element, bool is_propertyless) {
t_type* type = tfield->get_type();
while (type->is_typedef()) {
	type = ((t_typedef*) type)->get_type();
}
...
if (type->is_struct() || type->is_xception()) {
	...
	string nullable_name =
			nullable_ && !is_element && !field_is_required(tfield) ?
					name + ".Value" : name;
...
void t_csharp5_generator::generate_serialize_struct(ofstream& out,
	t_struct* tstruct, string prefix) {
(void) tstruct;
out << indent() << prefix << ".Write(oprot);" << endl;
}

void t_csharp5_generator::generate_serialize_container(ofstream& out,
	t_type* ttype, string prefix) {
scope_up(out);
...
void t_csharp5_generator::generate_serialize_map_element(ofstream& out,
	t_map* tmap, string iter, string map) {
...
void t_csharp5_generator::generate_serialize_set_element(ofstream& out,
	t_set* tset, string iter) {
t_field efield(tset->get_elem_type(), iter);
generate_serialize_field(out, &efield, "", true);
}

void t_csharp5_generator::generate_serialize_list_element(ofstream& out,
	t_list* tlist, string iter) {
t_field efield(tlist->get_elem_type(), iter);
generate_serialize_field(out, &efield, "", true);
}

void t_csharp5_generator::generate_property(ofstream& out, t_field* tfield,
	bool isPublic, bool generateIsset) {
generate_csharp_property(out, tfield, isPublic, generateIsset, "_");
}
void t_csharp5_generator::generate_csharp_property(ofstream& out,
	t_field* tfield, bool isPublic, bool generateIsset,
...
if ((nullable_ && !has_default) || (is_required)) {
	...
	indent(out) << (isPublic ? "public " : "private ")
			<< type_name(tfield->get_type(), false, false, true) << " "
	...
	if (use_nullable) {
		if (generateIsset) {
			indent(out) << "__isset." << tfield->get_name()
					<< " = value.HasValue;" << endl;
		}
		indent(out) << "if (value.HasValue) this."
				<< fieldPrefix + tfield->get_name() << " = value.Value;"
				<< endl;
	} else {
		if (generateIsset) {
			indent(out) << "__isset." << tfield->get_name() << " = true;"
					<< endl;
		}
		indent(out) << "this." << fieldPrefix + tfield->get_name()
				<< " = value;" << endl;
	}
	scope_down(out);
	scope_down(out);
...
std::string t_csharp5_generator::prop_name(t_field* tfield) {
string name(tfield->get_name());
name[0] = toupper(name[0]);
return name;
}

string t_csharp5_generator::type_name(t_type* ttype, bool in_container,
	bool in_init, bool in_param, bool is_required) {
(void) in_init;
while (ttype->is_typedef()) {
	ttype = ((t_typedef*) ttype)->get_type();
}
...
string postfix =
		(!is_required && nullable_ && in_param && ttype->is_enum()) ? "?" : "";
...
string t_csharp5_generator::base_type_name(t_base_type* tbase,
	bool in_container, bool in_param, bool is_required) {
(void) in_container;
...
string t_csharp5_generator::declare_field(t_field* tfield, bool init,
	std::string prefix) {
string result = type_name(tfield->get_type()) + " " + prefix
		+ tfield->get_name();
if (init) {
	t_type* ttype = tfield->get_type();
	while (ttype->is_typedef()) {
	...
	if (ttype->is_base_type() && field_has_default(tfield)) {
		ofstream dummy;
		result += " = "
				+ render_const_value(dummy, tfield->get_name(), ttype,
						tfield->get_value());
...
string t_csharp5_generator::function_signature(t_function* tfunction,
	string prefix) {
...
string t_csharp5_generator::function_signature_async_begin(
	t_function* tfunction, string prefix) {
string comma = (tfunction->get_arglist()->get_members().size() > 0 ? ", " : "");
return "IAsyncResult " + prefix + tfunction->get_name()
		+ "(AsyncCallback callback, object state" + comma
		+ argument_list(tfunction->get_arglist()) + ")";
}

string t_csharp5_generator::function_signature_async_end(t_function* tfunction,
	string prefix) {
t_type* ttype = tfunction->get_returntype();
return type_name(ttype) + " " + prefix + tfunction->get_name()
...
string t_csharp5_generator::function_signature_async(t_function* tfunction,
	string prefix) {
t_type* ttype = tfunction->get_returntype();
string task = "Task";
if (!ttype->is_void())
	task += "<" + type_name(ttype) + ">";
return task + " " + prefix + tfunction->get_name() + "Async("
		+ argument_list(tfunction->get_arglist()) + ")";
}

string t_csharp5_generator::argument_list(t_struct* tstruct) {
string result = "";
const vector<t_field*>& fields = tstruct->get_members();
vector<t_field*>::const_iterator f_iter;
bool first = true;
...
string t_csharp5_generator::type_to_enum(t_type* type) {
while (type->is_typedef()) {
	type = ((t_typedef*) type)->get_type();
}
...
void t_csharp5_generator::generate_csharp_docstring_comment(ofstream &out,
	string contents) {
...
void t_csharp5_generator::generate_csharp_doc(ofstream &out, t_field* field) {
...
void t_csharp5_generator::generate_csharp_doc(ofstream &out, t_doc* tdoc) {
if (tdoc->has_doc()) {
	generate_csharp_docstring_comment(out, tdoc->get_doc());
}
}

void t_csharp5_generator::generate_csharp_doc(ofstream &out,
	t_function* tfunction) {
...
std::string t_csharp5_generator::get_enum_class_name(t_type* type) {
string package = "";
...
THRIFT_REGISTER_GENERATOR(csharp5, "C# 5.0",
